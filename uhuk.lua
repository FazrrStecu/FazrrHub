...
local function bypassAntiCheat()
    pcall(function()
        -- Bypass common anti-cheat hooks
...
SubLabel.Text = "Premium Script | Steal An Egg"
...
-- Main Frame (Hidden by default, shows orb)
...
UserInputService.InputChanged:Connect(function(input)
    if orbDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        FloatingOrb.Position = UDim2.new(
            orbStartPos.X.Scale, orbStartPos.X.Offset + delta.X,
            orbStartPos.Y.Scale, orbStartPos.Y.Offset + delta.Y
        )
    end
end)
...
FloatingOrb.MouseButton1Click:Connect(function()
    toggleUI()
end)
...
createInput(autoPlacePage, "Place Species (comma separated)", function(v) end)
createInput(autoPlacePage, "Place Rarities (comma separated)", function(v) end)
createInput(autoPlacePage, "Place Mutations (comma separated)", function(v) end)
createInput(autoPlacePage, "Place Value (min)", function(v) end)
...
createInput(movePage, "Waypoint (X,Y,Z)", function(v) end)
...
createToggle(webhookPage, "Collect/Claim Alerts", false, function(v) end)
createToggle(webhookPage, "Pet Obtained Alerts", false, function(v) end)
createToggle(webhookPage, "Disconnect Alerts", false, function(v) end)
...
createButton(configPage, "Save Config", function()
    ...
    writefile(ConfigFolder .. "/config.json", data)
...
createButton(configPage, "Set As Auto Load", function() end)
createButton(configPage, "Delete Config", function()
    if isfile(ConfigFolder .. "/config.json") then
        delfile(ConfigFolder .. "/config.json")
    end
end)

createToggle(configPage, "Auto Load", false, function(v) Config.AutoLoad = v end)

--// TAB 8: JSON
local jsonPage = createTab("Config JSON")

createButton(jsonPage, "Export JSON", function()
    local data = HttpService:JSONEncode(Config)
    if setclipboard then
        setclipboard(data)
    end
    sendWebhook("Config Exported", "JSON berhasil di-copy ke clipboard!", 65280)
end)

createButton(jsonPage, "Import JSON", function()
...
local function stealEgg(egg)
    ...
    elseif Config.MovementMode == "Fly" then
        hrp.CFrame = CFrame.new(targetPos + Vector3.new(0, 3, 0))
    elseif Config.MovementMode == "Semi Teleport" then
        hrp.CFrame = CFrame.new(targetPos + Vector3.new(0, 3, 0))
        task.wait(0.05)
    end

    -- Random timing
    if Config.RandomTiming then
        task.wait(math.random(1, 3))
    end

    -- Grab egg
    pcall(function()
        local grabRemote = game:GetService("ReplicatedStorage"):FindFirstChild("GrabEgg")
        if grabRemote then
            grabRemote:FireServer(egg)
        end
    end)
    ...
    if base then
        ...
        pcall(function()
            local placeRemote = game:GetService("ReplicatedStorage"):FindFirstChild("PlaceEgg")
            if placeRemote then
                placeRemote:FireServer(egg)
            end
        end)
    end
end
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        if Config.AutoTreadmill then
            -- Auto treadmill logic
        end
    end
end)

--// GUARD IMMUNITY
if Config.GuardImmunity then
...
sendWebhook("Fazrr Hub Loaded", "Script berhasil di-load!", 65280)
