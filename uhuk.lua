--[[
    ...
    ║              Steal An Egg | DELTA EXECUTOR FIX               ║
    ║                  Owner: Tuan Fazrr                           ║
    ╚══════════════════════════════════════════════════════════════╝
]]
...
--// DELTA-SAFE LOCALPLAYER
local LocalPlayer
local ok, result = pcall(function() return Players.LocalPlayer end)
...
do
    ...
    if ok1 and r1 then
        ...
        if ok2 and r2 then
            ...
            _guiParent = LocalPlayer:WaitForChild("PlayerGui", 10) or LocalPlayer.PlayerGui
        end
    end
end

--// DESTROY OLD INSTANCE
for _, child in ipairs(_guiParent:GetChildren()) do
    if child.Name == "FazrrHub_Loading" or child.Name == "FazrrHub_Main" then
        child:Destroy()
    end
end
pcall(function()
...
--// ANTI-HANG HTTP PATCH (Delta fix)
local _rawHttp = request or http_request or (syn and syn.request)
if _rawHttp then
    local _orig = _rawHttp
    _rawHttp = function(opts)
        local done, res = false, nil
        task.spawn(function()
            local ok, r = pcall(_orig, opts)
            if ok then res = r end
            done = true
        end)
        local t0 = tick()
        while not done and (tick() - t0) < 5 do task.wait(0.1) end
        if not done then return {StatusCode = 0, Body = ""} end
        return res
    end
end
...
--// CONFIG
local Config = {
    AutoSteal = false, AutoPlace = false, AutoHatch = false, AutoTreadmill = false,
    GuardImmunity = false, RandomTiming = false,
    ESP_Egg = false, ESP_Pet = false, ESP_Player = false,
    BoostFPS = false, HideOwnerPlot = false, HideOtherEggs = false, NoClip = false,
    WalkSpeed = 16, JumpPower = 50, MovementMode = "Tween",
    SelectBy = "Rarity", PlaceMode = "All",
    WebhookURL = "", WebhookAlerts = false, AutoLoad = false,
}
...
pcall(function()
    ...
    for _, v in pairs(getconnections(LocalPlayer.Idled)) do v:Disconnect() end
        ...
        if req then
            req({
                ...
                Body = HttpService:JSONEncode({
                    embeds = {{title = title, description = message, color = color or 65280,
...
FloatingOrb.Active = true
FloatingOrb.Draggable = true
...
local function getGrabRemote() return findRemote("grab") or findRemote("steal") or findRemote("pick") end
local function getPlaceRemote() return findRemote("place") or findRemote("deposit") or findRemote("put") end
